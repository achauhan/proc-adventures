...
{
    61,
    "C:\\Users\\593091\\Documents\\code\\proc-adventures\\cursorArray.pc"
};


static unsigned int sqlctx = 154158003;
...
"select EMPNO ,ENAME ,MGR ,TO_CHAR(HIREDATE,'MM/DD/YYYY HH24:MI:SS') ,SAL  fr\
om EMP where JOB=:b0:b1           ";
...
{13,4128,1,27,0,
5,0,0,0,0,0,27,73,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,1,10,0,0,1,10,0,0,
36,0,0,2,110,0,9,98,0,0,1,1,0,1,0,1,9,0,0,
55,0,0,2,0,0,13,108,0,0,5,0,0,1,0,2,3,0,0,2,97,0,0,2,3,0,0,2,97,0,0,2,4,0,0,
90,0,0,2,0,0,15,121,0,0,0,0,0,1,0,
105,0,0,3,0,0,30,124,0,0,0,0,0,1,0,
120,0,0,4,0,0,31,170,0,0,0,0,0,1,0,
};


/********************************************************************************
* cursorarray.pc - This program explains using host array in CURSOR programming *
*                                                                               *
*********************************************************************************/
#include <stdio.h>
#include <stdlib.h>
...
struct { unsigned short len; unsigned char arr[10]; } host_job;
  ...
  {
    int host_empno[10];
    char host_ename[10][11];
    int host_mgr[10];
    char host_hiredate[10][23];
    double host_sal[10];
  } emp_info;
  
  struct 
  {
    short host_empno_i[10];
    short host_ename_i[10];
    short host_mgr_i[10];
    short host_hiredate_i[10];
    short host_sal_i[10];
  } emp_info_ind;
...
/* Forward declaration (used before definition in main) */
void print_records(int row);
...

{
  ...
  /* EXEC SQL DECLARE emp_cursor CURSOR FOR
        SELECT EMPNO, ENAME, MGR, TO_CHAR(HIREDATE,'MM/DD/YYYY HH24:MI:SS'), SAL
           FROM EMP
        WHERE JOB = :host_job:host_job_i; */ 
...

{
  ...
  sqlstm.sqindv[0] = (         void  *)&host_job_i;
  sqlstm.sqinds[0] = (         int  )0;
  sqlstm.sqharm[0] = (unsigned int  )0;
  sqlstm.sqadto[0] = (unsigned short )0;
  sqlstm.sqtdso[0] = (unsigned short )0;
  ...
  /* Initialize the number of rows returned */
  num_ret = 0;
...

{
     ...
     print_records(sqlca.sqlerrd[2] - num_ret);
     num_ret = sqlca.sqlerrd[2];        /* Set the row return number. */ 
  }
  
  /* Print any remaining rows from last fetch */
...
void print_records(int row)
{
  ...
  {
      printf("------------------------------------------------------- \n");
      printf("EMPLOYEE NO: %d \n",emp_info.host_empno[i]);
	  printf("EMPLOYEE NAME: %s \n",emp_info.host_ename[i]);
	  printf("MANAGER: %d \n",emp_info.host_mgr[i]);
	  printf("HIRE DATE: %s \n",emp_info.host_hiredate[i]);
	  printf("SALARY: %0.2f \n",emp_info.host_sal[i]); 
	  printf("------------------------------------------------------- \n");
  }
}
